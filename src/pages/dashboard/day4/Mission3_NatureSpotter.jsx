...
export const Mission3_AnimalMission = ({ onComplete, onBack }) => {
  const { user } = useAuth();
      ...
      {phase === 'instructions' ? (
        <div className="w-full max-w-xl animate-in zoom-in-95 duration-400">
          ...
          <div className="bg-white/5 border border-white/10 rounded-[2rem] p-6 sm:p-8 mb-8 shadow-2xl backdrop-blur-xl relative overflow-hidden">
            ...
            <h2 className="font-heading text-4xl sm:text-5xl font-black text-white mb-8 tracking-tight relative z-10">Animal Mission</h2>
            
            <div className="space-y-4 relative z-10">
              {[
                { emoji: '🍃', title: '1. A green leaf', desc: 'Can you find one nearby?' },
                { emoji: '🪨', title: '2. A smooth stone', desc: 'Look on the ground!' },
                { emoji: '☀️', title: '3. Your shadow', desc: 'Hold your hand up!' }
              ].map((step, i) => (
                <div key={i} className="flex items-center gap-5 p-4 rounded-2xl bg-black/40 border border-white/5 hover:border-green-500/30 transition-colors">
                  ...
                  <div>
                    <h3 className="font-heading font-black text-xl text-amber-400 tracking-wide">{step.title}</h3>
          ...
          <button
            ...
            className="w-full py-6 rounded-full font-heading font-black text-white text-2xl tracking-widest uppercase transition-all hover:scale-[1.02] shadow-[0_15px_30px_rgba(34,197,94,0.3)] relative overflow-hidden group border border-white/20"
          ...
          >
            ...
            <span className="relative z-10 text-shadow-sm">MY ANIMAL IS READY! 🐾</span>
          </button>
        </div>
      ) : (
        <div className="w-full max-w-xl animate-in slide-in-from-right-8 duration-400">
          ...
          <div className="text-center mb-8">
            <h2 className="font-heading text-4xl sm:text-5xl font-black text-white mb-2 tracking-tight">Upload Your Animal</h2>
            <p className="text-secondary-text font-ui">Take a photo of your animal find or drawing!</p>
          </div>
          ...
          >
            <div className={`w-full aspect-square bg-[#ececec] overflow-hidden border border-black/10 relative flex flex-col items-center justify-center ${preview ? '' : 'group'}`}>
              ...
              {preview ? (
                <>
                  <img src={preview} alt="Your nature find" className="w-full h-full object-cover animate-in fade-in zoom-in-95 duration-500" />
            ...
            <div className="absolute bottom-4 left-0 right-0 text-center pointer-events-none">
              <p className="font-marker text-2xl text-gray-600 opacity-60 transform rotate-1">My Animal Friend!</p>
            </div>
          </div>
